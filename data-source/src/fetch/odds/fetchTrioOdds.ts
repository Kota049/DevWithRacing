...
// 実行できていないので注意
async function fetchTrioOdds(page: Page): Promise<{ [key: string]: number }> {
  const targetLink = page.getByRole("link", { name: "3連複", exact: true });
  ...
  const oddsTableFrameList = await page.locator(".fuku3_list").all();
