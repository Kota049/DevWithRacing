...
async function fetchExactaOdds(page: Page): Promise<{ [key: string]: number }> {
  const targetLink = page.getByRole("link", { name: "馬単", exact: true });
  await targetLink.click();
  await page.waitForLoadState();

  let oddsList: { [key: string]: number } = {};
  const oddsTableFrameList = await page.locator(".umatan_list").all();
...
export default fetchExactaOdds;
